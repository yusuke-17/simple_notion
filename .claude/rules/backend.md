...
- **必須**: Goファイルを新規作成・全書き換えした直後は必ず `go fmt ./...` を実行する
- **理由**: ファイル末尾の余分な空行、インデント、import 順など `gofmt` の整形ルールに違反すると、pre-commit フック（`go fmt` チェック）でコミットが失敗する
- **タイミング**: Write ツールでファイルを書き出した直後、`go build` / `go test` の前に実行するのが安全
- **コマンド**: `cd backend && go fmt ./...`（または個別に `gofmt -w <file>.go`）
