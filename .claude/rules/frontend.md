# フロントエンド開発規約（Svelte 5 + TypeScript）

> Svelte 5/TypeScript専用の開発規約です。共通ルールは `common.md` も併せて参照してください。
...
```svelte
<script lang="ts">
  import { Bold, Italic, Trash2 } from 'lucide-svelte'
</script>
...
- **React Hooksパターンの直接移植**: `use{Name}`関数パターンは不要
